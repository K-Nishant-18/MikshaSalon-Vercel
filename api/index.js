...
var artists = sqliteTable("artists", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var authToken = process.env.DATABASE_AUTH_TOKEN;
...
var DatabaseStorage = class {
  ...
  async createGalleryItem(insertGallery) {
  ...
  async updateGalleryItem(id, update) {
  ...
  async updateTestimonial(id, update) {
...
async function registerRoutes(app) {
  ...
  app.get("/api/admin/content", async (_req, res) => {
    const content2 = await storage.getContent();
    res.json(content2);
  });
  app.post("/api/admin/content", async (req, res) => {
    ...
    const content2 = await storage.createContent(parsed.data);
    res.json(content2);
  });
  app.patch("/api/admin/content/:id", async (req, res) => {
    const id = parseInt(req.params.id);
    const content2 = await storage.updateContent(id, req.body);
    res.json(content2);
  });
}
  ...
  app.use((req, res, next) => {
    ...
    res.on("finish", () => {
      ...
      if (path.startsWith("/api")) {
        ...
        console.log(`${(/* @__PURE__ */ new Date()).toLocaleTimeString("en-US")} [express] ${logLine}`);
      }
    });
    next();
  });
